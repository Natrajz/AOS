...
const setState = function (el, top, once) {
  ...
  lastScrollTop = top + 40;
  let container = document.getElementById('transcroller-body');

  if(lastScrollTop > top && (top + container.offsetHeight) > el.position)  {
 ...
 * @return {void}
 */
const handleScroll = function ($elements, event, once) {
  if (event) {
